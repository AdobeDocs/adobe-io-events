...
- Adobe I/O Events sends the event payload with an additional field of `recipient_client_id`. The webhook consumer can match the payload's client id with his own client id to verify if he is the actual recipient of the event. This ensures he doesn't receive events he hasn't subscribed for.
