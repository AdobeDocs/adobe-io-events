...
- Adobe I/O Events sends the relative path of public key, which is served from our fixed Adobe domain [adobeioevents.com](https://adobeioevents.com), as webhook request headers.

**How it strengthens security**

- Adobe I/O Events uses the same public/private key pair generated by itself to sign all event payloads flowing through it's pipeline. The public keys are also hosted at it's own domain i.e. [adobeioevents.com](https://adobeioevents.com). This ensures that any malicious user can't use his own arbitrary public/private key pair to sign an event payload and send forged requests with his random public key url to your webhook endpoint. As, now the webhook consumer is using only the Adobe I/O Events generated public key served via the fixed Adobe domain and provided to him via the request header to verify the signature.
- Adobe I/O Events also supports custom events. So, consider a scenario that a malicious user `user-A` creates his own custom events, pushes them to Adobe IO Events, receives signed copy of the events and forwards them to `user-B`'s webhook. This can get `user-B` tricked because he can't identify that this event was sent by the malicious user and not Adobe I/O Events. So, he can get such events that he hasn't subscribed for. To avoid this scenario Adobe I/O Events sends the event payload with an additional field of `recipient_client_id` in it. The webhook consumer can match the payload's client id with his own client to verify if he is the actual recipient of the event and that the event is sent by us.

I/O Events generates two public-private key pairs and signs your event payload using two digital signatures following the steps below.
...
- once verified, your app should fetch the public key by forming and validating the url using the Adobe domain [adobeioevents.com](https://adobeioevents.com), and the relative path received from the request header
