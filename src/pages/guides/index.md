...
- signing all the event payloads digitally using a fixed public/private key pair generated by Adobe I/O Events and sending those digital signatures as request headers to your webhook
- sending the relative paths of public keys, which are served from our fixed adobe domain adobeioevents.com so that no one else can use their arbitrary public/private key pairs, as webhook request headers.
...
I/O Events also sends 2 public keys corresponding to the private keys used to generate the digital signatures. These public keys are stored in Cloud Front and are publicly accessible using our adobe domain adobeioevents.com. I/O Events sends only the relative paths of the public keys i.e. `/prod/keys/pub-key-<random-uuid>.pem` and they are available via the request header fields `x-adobe-public-key1-path` and  `x-adobe-public-key2-path` respectively.
...
- once verified, your app should fetch the public key by forming and validating the url using the adobe domain adobeioevents.com and the relative path received from the request header.
