...
- Adobe I/O Events sends the relative path of public key, which is served from our fixed Adobe subdomain [static.adobeioevents.com](https://static.adobeioevents.com), as webhook request headers.

**How it strengthens security**

- Adobe I/O Events uses the same public/private key pair generated by itself to sign all event payloads flowing through it's pipeline. The public keys are also hosted at it's own subdomain i.e. [static.adobeioevents.com](https://static.adobeioevents.com). This ensures that any malicious user can't use his own arbitrary public/private key pair to sign an event payload and send forged requests to your webhook endpoint.
...
- once verified, your app should fetch the public key by forming and validating the url using the Adobe subdomain [static.adobeioevents.com](https://static.adobeioevents.com), and the relative path received from the request header
